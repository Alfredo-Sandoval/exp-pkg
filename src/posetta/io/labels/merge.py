...
from posetta.core.annotations import LabeledFrame, Track
from posetta.core.skeleton import Skeleton
...
def _unify_against_base(base_labels: Labels, new_labels: Labels) -> None:
    ...
    def _map_video(video: VideoProtocol) -> VideoProtocol:
        if not video.filename:
            raise ValueError("Video missing filename while aligning bundles")
        abspath = str(Path(str(video.filename)).resolve())
        target = base_vid_by_abs.get(abspath)
        if target is not None:
            return target
        raise ValueError(
            "Unable to align videos between bundles. "
            "Provide absolute filenames in both bundles (no basename matching allowed)."
        )

    for lf in list(new_labels.labeled_frames):
        if lf.video is None:
            raise ValueError("LabeledFrame missing video while aligning bundles")
    ...
    for sug in list(new_labels.suggestions):
        if sug.video is None:
            raise ValueError("Suggestion missing video while aligning bundles")
...
def merge_container_dicts(dict_a: dict, dict_b: dict) -> None:
    ...
    for key in dict_b.keys():
        if key in dict_a:
            ...
            from posetta.core.path_registry import uniquify as _uniq
