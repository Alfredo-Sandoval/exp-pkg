...
"""
Utilities for managing metrics tables stored inside `.siesta` bundles.
...
def ensure_group(bundle_path: str | Path) -> None:
    """Guarantee that a `.siesta` bundle exposes a `/metrics` group.

    Args:
        bundle_path: Path to the .siesta bundle.
    """
...
def read_table(bundle_path: str | Path, name: str) -> pd.DataFrame:
    """Load a metrics table from a `.siesta` bundle.
...
def write_table_to_handle(
    ...
    *,
    ...
    Args:
        handle: Open `.siesta` HDF5 handle.
...
class MetricStore:
    """Read and write typed metrics tables from `.siesta` bundles."""
...
class LabelStore:
    """Read and write labeled-frame metadata tables from `.siesta` bundles."""
