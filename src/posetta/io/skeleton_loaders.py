...
Returns posetta.core.skeleton.Skeleton instances directly.
"""
...
from posetta.config.loaders import load_yaml_file
...
def build_sleap_skeleton(
    ...
    Returns:
        dict: Dictionary with ``name``, ``keypoints`` and ``links`` entries suitable
            for ``posetta.core.skeleton.Skeleton.from_dict``. Symmetry edges are
